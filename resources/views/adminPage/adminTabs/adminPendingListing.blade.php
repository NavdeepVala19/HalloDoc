    ...
    <form action="{{ route('admin.transfer.case') }}" method="POST">
        ...
        <div class="m-3">
            ...
            <div class="form-floating">
                <select class="form-select physicianRegions" name="region" id="floatingSelect" aria-label="Floating label select example">
                    <option selected>Regions</option>
                </select>
                <label for="floatingSelect">Narrow Search by Region</label>
            </div>
            <div class="form-floating">
                <select class="form-select selectPhysician" id="floatingSelect" aria-label="Floating label select example" name="physician">
...
<div class="main">
    <div class="heading-section d-flex align-items-center justify-content-between">
        ...
        <div class="admin-btn  d-flex gap-2">
            ...
            <form action="{{route('exportPending')}}" method="POST" id="filterExport" class="d-none">
                @csrf
    ...
    <div class="listing">
        <div class="search-section d-flex align-items-center  justify-content-between ">
            <form action="{{ route('searching', ['status' => 'pending', 'category' => request('category', 'all')]) }}" method="GET" class="d-flex align-items-center">
                {{-- @csrf --}}
            ...
            <div class="src-category d-flex gap-3 align-items-center">
                <a href="{{ route('admin.listing', ['category' => 'all', 'status' => 'pending']) }}" data-category="all" class="btn-all filter-btn">All</a>
                <a href="{{ route('admin.listing', ['category' => 'patient', 'status' => 'pending']) }}" data-category="patient" class="d-flex gap-2 filter-btn"> <i class="bi bi-circle-fill green"></i>Patient</a>
                <a href="{{ route('admin.listing', ['category' => 'family', 'status' => 'pending']) }}" data-category="family" class="d-flex gap-2 filter-btn"> <i class="bi bi-circle-fill yellow"></i>Family/Friend</a>
                <a href="{{ route('admin.listing', ['category' => 'business', 'status' => 'pending']) }}" data-category="business" class="d-flex gap-2 filter-btn"> <i class="bi bi-circle-fill red"></i>Business</a>
                <a href="{{ route('admin.listing', ['category' => 'concierge', 'status' => 'pending']) }}" data-category="concierge" class="d-flex gap-2 filter-btn"> <i class="bi bi-circle-fill blue"></i>Concierge</a>
            </div>
        </div>
                        ...
                        <td>
                            <div class="action-container">
                                ...
                                <div class="action-menu">
                                    <a href="{{ route('admin.view.case', $case->request->id) }}"><i class="bi bi-journal-arrow-down me-2 ms-3"></i>View Case</a>
                                    <a href="{{ route('provider.view.upload', ['id' => $case->request->id]) }}">
                                        <i class="bi bi-file-earmark-arrow-up-fill me-2 ms-3"></i>
                                        View Uploads
                                    </a>
                                    <button><i class="bi bi-journal-text me-2 ms-3"></i>View Notes</button>
                                    <button class="transfer-btn assign-case-btn" data-id="{{ $case->request->id }}"><i class="bi bi-send me-2 ms-3"></i>Transfer</button>
                                    <button class="clear-btn" data-id="{{ $case->request->id }}"><i class="bi bi-x-circle me-2 ms-3"></i>Clear
                                        Case</button>
                                    <button class="send-agreement-btn" data-id="{{ $case->request->id }}" data-request_type_id={{ $case->request->request_type_id }} data-phone_number={{ $case->request->phone_number }} data-email={{ $case->request->email }}><i class="bi bi-text-paragraph me-2 ms-3"></i>Send
                                        Agreement</button>
                                </div>
                            </div>
                        </td>
                ...
                <td>
                    <div class="action-container">
                        ...
                        <div class="action-menu">
                            ...
                            <button class="clear-btn" data-id="{{ $case->request->id }}"><i class="bi bi-x-circle me-2 ms-3"></i>Clear
                                Case</button>
                            <button class="send-agreement-btn" data-id="{{ $case->request->id }}" data-request_type_id={{ $case->request->request_type_id }}><i class="bi bi-text-paragraph me-2 ms-3"></i>Send
                                Agreement</button>
                        </div>
                    </div>
                </td>
