...
function fetchPaginatedResults(
    ...
    $.ajax({
        ...
        success: function (response) {
            $(".AdminNewListingPage").html(response.html); // Update results area
...
// search term query with ajax call
// $(document).on("submit", ".filter-section", function () {
//     var status = $(".nav-link.active").attr("id").split("-")[1];
//     var category = $(".filter-btn.active-filter").attr("data-category");

//     $.ajax({
//         url: `/admin/${status}/${category}`,
//         type: "POST",
//         success: function (response) {
